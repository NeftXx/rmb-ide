...
public class Intent extends IntentStorage {
    ...
    public <T> T showAnotherView(String fxml, String title) {
        try {
            ...
            debug.warning("Can't Open location : " + fxml);
            return null;
        }
    }
    ...
    public <T> T showAnotherView(String viewLocation, String title, Image icon) {
        try {
            FXMLLoader loader = new FXMLLoader(App.class.getResource(viewLocation));
            Parent root = loader.load();
            T controller = loader.getController();
            Stage stage = new Stage();
            stage.setScene(new Scene(root));
